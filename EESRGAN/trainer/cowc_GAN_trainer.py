...
class COWCGANTrainer:
    ...
    def test(self):
        for _, test_data in enumerate(self.data_loader):
            ...
            img_name = "asd"
            img_dir = '/home/razkey23/cowc-gan/EESRGAN/saved/testDir'
            #img_dir = "/home/jakaria/Super_Resolution/Filter_Enhance_Detect/saved/"
            #print(test_data['image_lq'])
            self.model.feed_data(test_data[0])
            self.model.test()
            ...
            print(sr_img)
            print(final_SR)
            save_img_path = os.path.join(img_dir, img_name+'_SR.png')
            save_img(sr_img, save_img_path)
            # Save final_SR images for reference
            save_img_path = os.path.join(img_dir, img_name+'_SR-Final.png')
            save_img(final_SR, save_img_path)
            
            save_img_path = os.path.join(img_dir, img_name+'_LQ.png')
